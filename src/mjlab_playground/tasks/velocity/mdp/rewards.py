...
def _is_forward_command_active(
  command: torch.Tensor, command_threshold: float
) -> torch.Tensor:
  """Gate rewards using only the x-command magnitude."""
  return (torch.abs(command[:, 0]) > command_threshold).float()
...
def track_heading_command(
  env: ManagerBasedRlEnv,
  std: float,
  command_name: str,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Reward tracking of heading targets instead of yaw-rate commands."""
  asset: Entity = env.scene[asset_cfg.name]
  ...
  heading_error = wrap_to_pi(heading_target - asset.data.heading_w)
  env.extras["log"]["Metrics/heading_error_mean"] = torch.mean(torch.abs(heading_error))
  reward = torch.exp(-torch.square(heading_error) / std**2)
  env.extras["log"]["Metrics/heading_reward_mean"] = torch.mean(reward)

  active_mask = torch.ones_like(heading_error, dtype=torch.bool)
  is_heading_env = getattr(command_term, "is_heading_env", None)
  if is_heading_env is not None:
    active_mask &= is_heading_env
  is_standing_env = getattr(command_term, "is_standing_env", None)
  if is_standing_env is not None:
    active_mask &= ~is_standing_env

  return torch.where(active_mask, reward, torch.ones_like(reward))
...
def feet_clearance(
  ...
  if command_name is not None:
    command = env.command_manager.get_command(command_name)
    if command is not None:
      cost = cost * _is_forward_command_active(command, command_threshold)
  return cost
...
class feet_swing_height:
  ...
  def __call__(
    ...
    active = _is_forward_command_active(command, command_threshold)
