...
def commands_vel(
  ...
  for stage in velocity_stages:
    if env.common_step_counter > stage["step"]:
      ...
      if "heading" in stage and stage["heading"] is not None:
        cfg.ranges.heading = stage["heading"]
  return {
    "lin_vel_x_min": torch.tensor(cfg.ranges.lin_vel_x[0]),
    "lin_vel_x_max": torch.tensor(cfg.ranges.lin_vel_x[1]),
    "lin_vel_y_min": torch.tensor(cfg.ranges.lin_vel_y[0]),
    "lin_vel_y_max": torch.tensor(cfg.ranges.lin_vel_y[1]),
    "heading_min": torch.tensor(
      cfg.ranges.heading[0] if cfg.ranges.heading is not None else 0.0
    ),
    "heading_max": torch.tensor(
      cfg.ranges.heading[1] if cfg.ranges.heading is not None else 0.0
    ),
  }
